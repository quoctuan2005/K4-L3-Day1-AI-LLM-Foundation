...
def compare_models(prompt: str) -> dict:
    ...
    gpt4o_mini_text, gpt4o_mini_latency = call_openai_mini(prompt)
    gpt4o_cost_estimate = (len(gpt4o_text.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    return {
        "gpt4o_response": gpt4o_text,
        "mini_response": gpt4o_mini_text,
        "gpt4o_latency": gpt4o_latency,
        "mini_latency": gpt4o_mini_latency,
...
def run_assistant(
    ...
    if get_input is None or get_input is input:
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    widths = [30, 42, 42, 16, 14]
    header_row = " | ".join(f"{h:<{w}}" for h, w in zip(headers, widths))
    separator = "-+-".join("-" * w for w in widths)
    rows = [header_row, separator]
    for r in results:
        p = truncate(r.get("prompt", ""), 30)
        g_resp = truncate(r.get("gpt4o_response", ""), 42)
        m_resp = truncate(r.get("mini_response", ""), 42)
        g_lat = f"{r.get('gpt4o_latency', 0.0):.2f}s"
        m_lat = f"{r.get('mini_latency', 0.0):.2f}s"
        row = f"{p:<30} | {g_resp:<42} | {m_resp:<42} | {g_lat:<16} | {m_lat:<14}"
        rows.append(row)
    return "\n".join(rows)
